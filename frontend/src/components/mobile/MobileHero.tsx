...
export default function MobileHero({ onOpenBooking }: MobileHeroProps) {
  const [splitPct, setSplitPct] = useState<number>(0);
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      requestAnimationFrame(() => {
        ...
        // The parting doors effect activates between 0.15vh and 0.85vh
        const start = vh * 0.15;
        const end = vh * 0.90;
        ...
        // Direct hardware-accelerated transform without React re-render overhead
        ...
        if (Math.abs(pct - lastPctRef.current) > 2) {
          lastPctRef.current = pct;
          setSplitPct(pct);
        }

        setIsInView(scrollY < vh * 1.5);
      });
    };
  ...
  return (
    <div id="mobile-hero-top" ref={heroRef} className="relative w-full">
      {/* 1. High-Quality Golden Trishula Parting Doors (Fixed Background Layer) */}
      <div
      ...
      {/* 2. Mobile Hero Content Section (Overlaid on top of background) */}
      <section className="relative z-20 min-h-[100dvh] flex flex-col justify-between items-center text-center px-4 sm:px-6 py-20 sm:py-24">
